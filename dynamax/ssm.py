    ...
    * :meth:`fit_sgd` run SGD to minimize the *negative* marginal log probability.
    ...
    def fit_em(
        ...
        *Note:* ``emissions`` *and* ``inputs`` *can either be single sequences or batches of sequences.*
        ...
        EM uses ``lax.scan``: parameters and M-step state must retain the same
        pytree structure, array shapes, and dtypes from initialization through
        every M-step. Caller-supplied parameters must use the representation
        expected by the model's M-step.
